...
const Modal = ({ children, close }) => {
  ...
  return ReactDOM.createPortal(<ModalBody close={close}>{children}</ModalBody>, el);
};

const ModalBody = ({ children, close }) => {
  return (
    <div className="dodo-modal-body">
      {children}
      <CloseBtn close={close}></CloseBtn>
    </div>
  );
};
const CloseBtn = ({ close }) => {
  return (
    <i className="modal-closeBtn" onClick={close}>
      <svg
        width="25"
        height="25"
        viewBox="0 0 25 25"
